...
import com.example.nasa_app.ViewModel.SpiritViewModel
...
class BlankFragmentSpirit : Fragment(),OnNasaItemClickListener {


    var recyclerView: RecyclerView? = null
    var progressBarSpirit: ProgressBar? = null
    var gridLayoutManager: GridLayoutManager? = null
    var spiritViewModel: SpiritViewModel? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val view = inflater.inflate(R.layout.fragment_blank_spirit, container, false)
        return view
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        init()
        spiritViewModel = ViewModelProvider(viewModelStore,
                ViewModelProvider.AndroidViewModelFactory(requireActivity().application)).get(SpiritViewModel::class.java)
    ...
    private fun init() {
        recyclerView = view!!.findViewById(R.id.recyclerView_spirit)
        gridLayoutManager = GridLayoutManager(context, 2)
        gridLayoutManager!!.orientation = LinearLayoutManager.VERTICAL
        progressBarSpirit = view!!.findViewById(R.id.progressBarSpirit)
    }
    ...
    private fun getPhotos(currentPage: Int){
        CoroutineScope(Dispatchers.IO).launch {
            spiritViewModel!!.getSpiritPhotoList(currentPage.toString())
            if (spiritViewModel!!.getSpiritphoto.value !=null){
                loadroversphotos(spiritViewModel!!.getSpiritphoto.value!!.photos)
            }
            hideProgressBar();
        }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if (camera!=null){
            CoroutineScope(Dispatchers.IO).launch {
                spiritViewModel!!.getFilterCameraOfSpirit(camera)
                if (spiritViewModel!!.filterCameraOfSpirit.value!=null){
                    loadroversphotos(spiritViewModel!!.filterCameraOfSpirit.value!!.photos)
                }
                hideProgressBar();
            }
        }
    ...
    fun showProgressBar() {
        Thread(Runnable {
            requireActivity().runOnUiThread(java.lang.Runnable {
                progressBarSpirit!!.visibility = View.VISIBLE
            })
        }).start()
    }
    ...
    fun hideProgressBar() {
        Thread(Runnable {
            requireActivity().runOnUiThread(java.lang.Runnable {
                if (progressBarSpirit != null && progressBarSpirit!!.isShown) {
                    progressBarSpirit!!.visibility = View.GONE
                }
            })
        }).start()
    }
